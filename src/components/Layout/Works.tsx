  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {projects.map((exp, i) => (
          <ProjectCard key={i} exp={exp} index={i} />
        ))}
      </div>
    </section>
  );
...
function ProjectCard({ exp, index }: { exp: any; index: number }) {
  const [activeImg, setActiveImg] = useState(0);

  return (
    <div className={`group border-b border-white/5 py-12 md:py-20 flex flex-col gap-12 xl:gap-16 hover:bg-blue-500/[0.01] transition-all duration-500 items-center ${index % 2 === 0 ? "xl:flex-row" : "xl:flex-row-reverse"}`}>
